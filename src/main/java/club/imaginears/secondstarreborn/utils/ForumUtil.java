...
public class ForumUtil {
    ...
    public void initialize() throws IOException, SQLException {
        DatabaseConnection sql = SecondStar.getConfigUtil().getSQLInfo();
    ...
    public void linkAccount(Player player, String email) {
        if (!isConnected()) {
            player.sendMessage(NamedTextColor.RED + "Could not connect to database, please try again later!");
            return;
        }
        try (Connection connection = connectionPool.getConnection()) {
            ...
            if (checkMcNotLinkedResult.next()) {
                player.sendMessage(NamedTextColor.RED + "Your Minecraft account is already linked to a Forum account. To unlink, type /link cancel.");
            ...
            if (!memberIdResult.next()) {
                player.sendMessage(NamedTextColor.RED + "There is no forum account with that email address!");
                player.sendMessage(NamedTextColor.YELLOW + "Register for our forums here: " + NamedTextColor.GOLD + "https://forums.palace.network/register/");
            ...
            if (!checkNotLinkedResult.next()) {
                player.sendMessage(NamedTextColor.RED + "There was an error, please try again later!");
            ...
            if ((uuidField != null && !uuidField.isEmpty()) || (usernameField != null && !usernameField.isEmpty())) {
                player.sendMessage(NamedTextColor.RED + "This forum account is already linked to a different Minecraft account!");
                return;
            }

            String linkingCode = getRandomNumberString();
            SecondStar.getMongoHandler().setForumLinkingCode(player.getUniqueId(), member_id, linkingCode);
            ...
            player.sendMessage(NamedTextColor.GREEN + "Okay, you're almost finished linking! Visit your Forum Profile and look for the six-digit Linking Code. When you've found it, run " +
                    NamedTextColor.YELLOW + "/link confirm [six-digit code].");
            player.sendMessage(new ComponentBuilder("If you need help, check out our guide ").color(NamedTextColor.GREEN)
                    .append("here!").color(NamedTextColor.YELLOW)
                    .event(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                            new ComponentBuilder("Click to visit ").color(NamedTextColor.GREEN)
                                    .append("https://forums.palace.network/topic/6141-link-your-minecraft-and-forum-accounts/")
                                    .color(NamedTextColor.YELLOW).create()))
            ...
            player.sendMessage(NamedTextColor.RED + "There was an error, please try again later!");
            SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error linking forum account", e);
        }
    }

    public void unlinkAccount(Player player) {
        try {
            int member_id = SecondStar.getMongoHandler().getForumMemberId(player.getUniqueId());
            if (SecondStar.getMongoHandler().getForumLinkingCode(player.getUniqueId()) != null || member_id < 0) {
                player.sendMessage(NamedTextColor.RED + "Your Minecraft and Forums accounts are not currently linked!");
                return;
            }
            player.sendMessage(NamedTextColor.GREEN + "Unlinking your Minecraft and Forums accounts...");
            try (Connection connection = connectionPool.getConnection()) {
                ...
                SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error unlinking forum account", e);
            }
            SecondStar.getMongoHandler().unlinkForumAccount(player.getUniqueId());
        } catch (Exception e) {
            player.sendMessage(NamedTextColor.RED + "There was an error, please try again later!");
            SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error unlinking forum account", e);
        }
        player.sendMessage(NamedTextColor.GREEN + "Your Minecraft and Forums accounts are no longer linked.");
    }
    ...
    public void confirm(Player player, String code) {
        String correctCode = SecondStar.getMongoHandler().getForumLinkingCode(player.getUniqueId());
        if (correctCode == null) {
            player.sendMessage(NamedTextColor.RED + "Before you can confirm, first you need to start the linking process: /link [email address]");
            return;
        }
        if (!code.equals(correctCode)) {
            player.sendMessage(NamedTextColor.RED + "That isn't the right code!");
            return;
        }
        player.sendMessage(NamedTextColor.GREEN + "Code accepted! We're finishing the linking process right now...");
        int member_id = SecondStar.getMongoHandler().getForumMemberId(player.getUniqueId());
        if (member_id < 0) {
            player.sendMessage(NamedTextColor.RED + "Uh oh, there was a problem! (Error code 121)");
            return;
        }
        try (Connection connection = connectionPool.getConnection()) {
            ...
            if (player.getRank().getRankId() < Rank.DEVELOPER.getRankId()) {
                ...
                player.sendMessage(NamedTextColor.RED + "Note: Since your rank is " + player.getRank().getFormattedName() + NamedTextColor.RED + ", your forum group will not be automatically set.");
            }

            //Unset Linking Code value on mongo document
            SecondStar.getMongoHandler().unsetForumLinkingCode(player.getUniqueId());
        } catch (SQLException e) {
            player.sendMessage(NamedTextColor.RED + "There was an error, please try again later!");
            SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error confirming forum account", e);
            return;
        }
        player.sendMessage(NamedTextColor.GREEN + "All done! Your Minecraft and Forums accounts are now linked.");
    }
    ...
    public void updatePlayerName(UUID uuid, int member_id, String newUsername) {
        try (Connection connection = connectionPool.getConnection()) {
            ...
            SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error updating name on forum account", e);
        }
    }
    ...
    public void updatePlayerRank(UUID uuid, int member_id, Rank rank, Player player) {
        if (rank.getRankId() >= Rank.DEVELOPER.getRankId()) {
            if (player != null)
                player.sendMessage(NamedTextColor.RED + "Note: Since your rank is " + player.getRank().getFormattedName() +
                        NamedTextColor.RED + ", your forum group will not be automatically set.");
            return;
        }
        try (Connection connection = connectionPool.getConnection()) {
            ...
            SecondStar.getProxyServer().getLogger().log(Level.SEVERE, "Error updating rank on forum account", e);
        }
    }
